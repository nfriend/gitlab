  ...
  if (assetsList.length > 0) {
    await Promise.all(
      assetsList.map(({label, alt, url}) => {
        ...
        const assetLinkUrl = urlJoin(gitlabApiUrl, `/projects/${encodedRepoId}/releases/${encodedGitTag}/assets/links`);
        const requestParams = {name: label || alt, url: urlJoin(gitlabUrl, repoId, url)};
        debug('Making a request to %s with params: %O', assetLinkUrl, requestParams);
        return got.post(assetLinkUrl, {
          ...apiOptions,
          json: requestParams
        });
      })
